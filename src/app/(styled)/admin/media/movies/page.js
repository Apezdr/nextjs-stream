...
async function MoviesAdminPage({ searchParams }) {
  ...
  const { items, total, page: resolvedPage, pageSize } = await listAdminMovies({ search: q, page, pageSize: size, sort, dir, serverId, quality, year, video, hdr })

  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <h1 className="mb-6 text-2xl font-semibold text-gray-900">Movies</h1>
      <MediaBrowser type="movie" items={items} total={total} page={resolvedPage} pageSize={pageSize} q={q} sort={sort} dir={dir} servers={servers} serverId={serverId} quality={quality} year={year} video={video} hdr={hdr} />
    </div>
  )
}

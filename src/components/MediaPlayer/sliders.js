...
// Slider CSS vars are raw 0-100 numbers, hence the *1% calc.
const thumbClass =
  'absolute left-[calc(var(--media-slider-fill)*1%)] top-1/2 z-20 h-[15px] w-[15px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 ring-white/40 transition-opacity group-data-[interactive]:opacity-100 group-data-[dragging]:ring-4 will-change-[left]'
  ...
  return (
    <VolumeSlider.Root className="volume-slider group relative mx-[7.5px] inline-flex h-10 w-full max-w-[80px] cursor-pointer touch-none select-none items-center outline-none">
      <VolumeSlider.Track className="relative z-0 h-[5px] w-full rounded-sm bg-white/30">
        <VolumeSlider.Fill className="absolute h-full w-[calc(var(--media-slider-fill)*1%)] rounded-sm bg-blue-300 will-change-[width]" />
      </VolumeSlider.Track>
  ...
  return (
    <TimeSlider.Root className="time-slider group relative mx-[7.5px] inline-flex h-10 w-full cursor-pointer touch-none select-none items-center outline-none">
      <TimeSlider.Track className="relative z-0 h-[5px] w-full overflow-hidden rounded-sm bg-white/30">
        <TimeSlider.Buffer className="absolute h-full w-[calc(var(--media-slider-buffer)*1%)] rounded-sm bg-white/50 will-change-[width]" />
        <TimeSlider.Fill className="absolute z-10 h-full w-[calc(var(--media-slider-fill)*1%)] rounded-sm bg-red-500 will-change-[width]" />
      </TimeSlider.Track>
      <ChapterTicks />

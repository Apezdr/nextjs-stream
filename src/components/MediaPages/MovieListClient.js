...
// Memoized individual movie card component
    ...
    >
      <Link href={`movie/${encodeURIComponent(movie.title)}`} className="group" scroll={true}>
        ...
        {movie.metadata?.release_date ? (
          ...
          >
            <p className="pointer-events-none mt-2 block text-sm font-medium text-gray-200 text-center">
              {new Date(movie.metadata.release_date).toLocaleDateString()}
            </p>
          </PageContentAnimatePresence>
        ) : null}
        <PageContentAnimatePresence
...
export default function MovieListClient({ initialFilters, initialData }) {
  ...
  useEffect(() => {
    ...
    // Debounce fetch to prevent request pileup (shorter delay than URL for responsiveness)
    ...
    }, 150); // Shorter debounce for data fetch (feels more responsive)
  ...
  useEffect(() => {
    ...
    urlDebounceRef.current = setTimeout(() => {
      ...
      if (filters.resolutions.length > 0) {
        params.set('res', filters.resolutions.join(','));
      }
      if (page > 1) {
  ...
  const { items: currentMovies, totalCount, totalPages, filterOptions } = data;
  const { availableGenres, availableHdrTypes, availableResolutions } = filterOptions;
  ...
  const handleResolutionToggle = useCallback((resolution) => {
    setFilters(prev => {
      const exists = prev.resolutions.includes(resolution);
      const resolutions = exists
  ...
  const handleClearFilters = useCallback(() => {
    setFilters(prev => ({ ...prev, genres: [], hdrTypes: [], resolutions: [] }));
    setPage(1);
  }, []);
      ...
      <li className="col-span-full mb-6 border-b border-gray-700 pb-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-col gap-3 w-full md:w-auto">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 max-w-3xl">
                {availableResolutions && availableResolutions.length > 0 ? availableResolutions.map(resolution => (
                  <FilterButton
                    key={resolution}
                    text={resolution}
                    isSelected={filters.resolutions.includes(resolution)}
                    onToggle={handleResolutionToggle}
                  />
                )) : (
                  <div className="text-sm text-gray-400">No resolution data available</div>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex flex-row gap-2 items-center">
            {(filters.genres.length > 0 || filters.hdrTypes.length > 0 || filters.resolutions.length > 0) ? (
              <div className="flex items-center">
                <span className="text-sm text-gray-200">
                  Showing {totalCount} movies with selected filters
                  {filters.genres.length > 0 && ` (Genres: ${filters.genres.join(', ')})`}
                  {filters.hdrTypes.length > 0 && ` (HDR: ${filters.hdrTypes.join(', ')})`}
                  {filters.resolutions.length > 0 && ` (Res: ${filters.resolutions.join(', ')})`}
              ...
              <span className="text-sm text-gray-200">
                Showing {totalPages > 0 ? `${page} of ${totalPages}` : '0'} pages ({totalCount} movies)
              </span>
            )}
            {/* Share button */}
            {(filters.genres.length > 0 || filters.hdrTypes.length > 0 || filters.resolutions.length > 0 || filters.sortOrder !== 'newest' || page > 1) && (
              <button

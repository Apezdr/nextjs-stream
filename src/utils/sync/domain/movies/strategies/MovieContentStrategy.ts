    ...
    try {
      ...
      for (const [language, subtitleData] of Object.entries(subtitles)) {
        if (subtitleData && typeof subtitleData === 'object' && 'url' in subtitleData) {
          ...
          const autoGenerated = Boolean((subtitleData as any).autoGenerated)
          const pending = Boolean((subtitleData as any).pending)

          // Auto-caption stubs ship a fully-qualified URL from the scanner
          // (CAPTIONS_TRACK_URL_PREFIX + path) — skip prefix stripping + createFullUrl
          // so we don't double-prefix or rewrite the absolute URL.
          let fullUrl: string
          if (autoGenerated && pending) {
            ...
            fullUrl = UrlBuilder.createFullUrl(cleanPath, context.serverConfig)
          }

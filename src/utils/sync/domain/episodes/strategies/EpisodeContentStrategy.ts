...
export class EpisodeContentStrategy implements SyncStrategy {
  ...
  private extractCaptions(
    ...
    for (const [langName, subtitleData] of Object.entries(subtitles)) {
      ...
      const autoGenerated = Boolean(sd.autoGenerated)
      const pending = Boolean(sd.pending)

      // Auto-caption stubs ship a fully-qualified URL — skip prefix/createFullUrl.
      const fullUrl = autoGenerated && pending
        ? rawUrl
        : UrlBuilder.createFullUrl(this.stripPrefix(rawUrl, context), context.serverConfig)
      ...
      if (!existing || existing.url !== fullUrl || existing.lastModified !== sd.lastModified
          ...
          || Boolean(existing.autoGenerated) !== autoGenerated
          || Boolean(existing.pending) !== pending) {
